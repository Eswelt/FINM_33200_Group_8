...
from modeling import build_three_class_classifier
...
def _feature_columns(data: pd.DataFrame, feature_set: str) -> Tuple[list, str]:
    return pipeline_feature_columns(data, feature_set)


def _positive_class_probability(model, x_frame: pd.DataFrame, label: int) -> np.ndarray:
...
def evaluate_volatility_thresholds(
    ...
    for k in k_values:
        ...
        for feature_set in feature_sets:
            numeric_columns, text_column = _feature_columns(data, feature_set)
            columns = numeric_columns + ([text_column] if text_column else [])
            for fold, train, test in splits:
                model = build_three_class_classifier(
                    train["target_vol_adj_3class"],
