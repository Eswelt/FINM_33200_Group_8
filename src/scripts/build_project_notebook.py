...
def _source_map() -> pd.DataFrame:
    rows = [
        ...
        ("CLI orchestration", "src/cli.py", "Command surface used by pydoit; now reuses cached prices when available."),
        ("Configuration", "src/config.py", "Research defaults, dates, paths, thresholds, feature-set names."),
        ("Data adapters", "src/data/prices.py, src/data/weather.py, src/data/usda.py", "Price pulls, weather cache/demo adapter, USDA text adapter."),
        ("Feature panel", "src/features.py", "Weekly price, calendar, weather, text, AI feature joins."),
        ("Validation/modeling utilities", "src/validation.py, modeling.py", "Shared walk-forward splits and reusable sklearn pipelines."),
        ("Main target test", "src/price_target_tests.py", "Fixed 2 percent arithmetic-return target plus return-regression diagnostics."),
        ("Return strategy", "src/expected_return_strategy.py", "Expected-return models, trading threshold, strategy returns."),
...
def _project_directory_map() -> pd.DataFrame:
    rows = [
        ...
        ("src/", "cli.py, features.py, validation.py, modeling.py", "Forecasting package and command surface used by pydoit."),
        ("src/data/", "prices.py, weather.py, usda.py", "Data adapters for price, weather, and USDA release inputs."),
        ("src/text/", "wwcb*.py, ai_features.py", "Optional WWCB parsing and AI feature extraction helpers."),
        ("src/scripts/", "build_project_notebook.py, run_horizon_robustness.py, WWCB scripts", "One-off report, robustness, and text-processing scripts."),
...
def _workflow_output_map() -> pd.DataFrame:
    workflows = [
        (
            "research",
            [
                REPORTS_DIR / "price_target_tests.json",
                REPORTS_DIR / "price_target_predictions.csv",
                REPORTS_DIR / "expected_return_metrics.json",
                REPORTS_DIR / "expected_return_predictions.csv",
                REPORTS_DIR / "volatility_metrics.json",
                REPORTS_DIR / "volatility_predictions.csv",
                REPORTS_DIR / "threshold_selection.json",
                REPORTS_DIR / "threshold_selection_predictions.csv",
                DEFAULT_NOTEBOOK,
            ],
        ),
        (
...
def _build_report_context() -> dict:
    ...
    price_metrics = _metrics_frame(_read_json(REPORTS_DIR / "price_target_tests.json"))
    expected_metrics = _metrics_frame(_read_json(REPORTS_DIR / "expected_return_metrics.json"))
    volatility_metrics = _metrics_frame(_read_json(REPORTS_DIR / "volatility_metrics.json"))
    binary_metrics = _metrics_frame(_read_json(REPORTS_DIR / "metrics.json"))
    threshold_metrics = _metrics_frame(_read_json(REPORTS_DIR / "threshold_selection.json"))
    ...
    oos = _prediction_summary(REPORTS_DIR / "price_target_predictions.csv")
...
def _plot_fixed_confusion_matrices(context: dict) -> Optional[Path]:
    metrics = _read_json(REPORTS_DIR / "price_target_tests.json") or {}
...
The project treats CORN ETF forecasting as a weekly trading-signal problem. The main target is a fixed 2 percent next-week arithmetic return band:
...
The implementation stores next-week log returns and converts the arithmetic band to log bounds before assigning labels. The main comparison is `price_only` versus `price_calendar` under expanding walk-forward validation after `2022-12-31`. Auxiliary checks include expected-return trading, volatility-adjusted target selection, and a secondary binary up/down pipeline.
"""
        ),
