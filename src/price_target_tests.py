...
from features import pipeline_feature_columns, price_feature_columns
from modeling import build_regressor, build_three_class_classifier
...
def arithmetic_threshold_to_log_bounds(threshold: float) -> Tuple[float, float]:
    """Convert a decimal arithmetic return band to comparable log-return bounds."""
    if threshold < 0 or threshold >= 1:
        raise ValueError("threshold must be in [0, 1).")
    return float(np.log1p(-threshold)), float(np.log1p(threshold))


def add_three_class_return_target(panel: pd.DataFrame, threshold: float = 0.02) -> pd.DataFrame:
    """Add -1/0/1 classes for next-week arithmetic returns below/inside/above a threshold."""
    frame = panel.copy()
    returns = frame["target_log_return_next"]
    down_log_threshold, up_log_threshold = arithmetic_threshold_to_log_bounds(threshold)
    frame["target_return_3class"] = np.select(
        [returns <= down_log_threshold, returns >= up_log_threshold],
        [-1, 1],
        default=0,
    )
...
def _three_class_metrics(predictions: pd.DataFrame, threshold: float, feature_set: str) -> Dict[str, float]:
    ...
    return {
        "target": f"next_week_arithmetic_return_3class_{threshold:.1%}_bands",
        "model": f"{feature_set}_logit",
        "feature_set": feature_set,
        "threshold": float(threshold),
        "down_log_threshold": float(down_log_threshold),
        "up_log_threshold": float(up_log_threshold),
        "n_test": int(len(predictions)),
        "n_folds": int(predictions["fold"].nunique()),
...
def run_price_only_target_tests(
    ...
    """Run return-regression diagnostics and fixed-threshold three-class tests."""
    ...
    regression_frames = []
    class_frames = []
    splits = list(walk_forward_splits(data, split_date, test_window_weeks, retrain_step_weeks))
    if not splits:
    ...
    for feature_set in feature_sets:
        ...
        for fold, train, test in splits:
            reg_model = build_regressor("ridge", numeric_columns=numeric_columns, text_column=text_column)
            reg_model.fit(train[fit_columns], train["target_log_return_next"])
            y_pred_return = reg_model.predict(test[fit_columns])
            ...
            class_model = build_three_class_classifier(
