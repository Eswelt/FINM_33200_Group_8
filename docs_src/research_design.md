# CORN ETF Trading Signal Research Design
...
Current decision: the main target is a fixed-threshold three-class next-week arithmetic return label.

For each week `t`:

```text
r_{t+1} = log(close_{t+1} / close_t)
down_log_bound = log(1 - 0.02)
up_log_bound   = log(1 + 0.02)
```

Target:

```text
Y_t =  1 if r_{t+1} >= up_log_bound
Y_t =  0 if down_log_bound < r_{t+1} < up_log_bound
Y_t = -1 if r_{t+1} <= down_log_bound
```
...
- In the 2023-2026 OOS window, it gives 19 down weeks, 132 flat weeks, and 24 up weeks with the arithmetic 2% band converted to log-return bounds.
